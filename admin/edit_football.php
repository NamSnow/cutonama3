...
if (isset($_GET['id'])) {
    $football_news_id = filter_var($_GET['id'], FILTER_SANITIZE_NUMBER_INT);

    // Fetch the football news article from the database
    $stmt = $pdo->prepare("SELECT * FROM football_news WHERE football_news_id = ?");
    $stmt->execute([$football_news_id]);
    $football_news = $stmt->fetch(PDO::FETCH_ASSOC);

    if (!$football_news) {
        $error = 'Tin tức bóng đá không tồn tại.';
    }
} else {
    $error = 'Không có ID tin tức bóng đá được cung cấp.';
}

// Handle form submission for updating football news
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $football_news) {
    $title = trim($_POST['title']);
    $summary = trim($_POST['summary']);
    $content = trim($_POST['content']);
    $is_published = isset($_POST['is_published']) ? 1 : 0;
    $current_image = $football_news['image']; // Get existing image path
    ...
    if (empty($title) || empty($summary) || empty($content)) {
        ...
        if (empty($error)) {
            try {
                $stmt = $pdo->prepare("UPDATE football_news SET title = ?, summary = ?, content = ?, image = ?, is_published = ? WHERE football_news_id = ?");
                $stmt->execute([$title, $summary, $content, $image_path, $is_published, $football_news_id]);
                $success = 'Tin tức bóng đá đã được cập nhật thành công!';
                // Refresh football news data to show updated info immediately
                $stmt = $pdo->prepare("SELECT * FROM football_news WHERE football_news_id = ?");
                $stmt->execute([$football_news_id]);
                $football_news = $stmt->fetch(PDO::FETCH_ASSOC);
...
<head>
    ...
    <title>Chỉnh sửa Tin tức bóng đá</title>
    ...
    <style>
        ...
        .form-input, .form-textarea, .form-select {
            ...
            @apply shadow-sm bg-white border border-gray-300 rounded-lg py-3 px-4 text-gray-800 leading-tight focus:outline-none focus:ring-4 focus:ring-blue-300 focus:border-blue-500 transition duration-300 ease-in-out; /* Blue focus for football theme */
        }
        .form-checkbox {
            /* Changed colors for light theme */
            @apply h-5 w-5 text-blue-600 focus:ring-blue-500 border-gray-400 rounded-md bg-white; /* Blue checkbox for football theme */
        }
...
<body class="bg-gray-50 min-h-screen text-gray-800">
    ...
    <div class="container mx-auto py-12 px-4 sm:px-6 lg:px-8 max-w-4xl">
        ...
        <div class="bg-white shadow-2xl rounded-xl p-8 mb-8 border border-gray-200">
            ...
            <?php if ($football_news): ?>
                <form action="edit_football.php?id=<?php echo htmlspecialchars($football_news['football_news_id']); ?>" method="POST" enctype="multipart/form-data" class="space-y-7">
                    <div>
                        <label for="title" class="block text-sm font-medium text-gray-700 mb-2">Tiêu đề:</label>
                        <input type="text" id="title" name="title" value="<?php echo htmlspecialchars($football_news['title']); ?>" required class="form-input w-full" placeholder="Nhập tiêu đề tin tức bóng đá">
                    </div>

                    <div>
                        <label for="summary" class="block text-sm font-medium text-gray-700 mb-2">Tóm tắt:</label>
                        <textarea id="summary" name="summary" rows="4" required class="form-textarea w-full" placeholder="Nhập tóm tắt tin tức bóng đá"><?php echo htmlspecialchars($football_news['summary']); ?></textarea>
                    </div>

                    <div>
                        <label for="content" class="block text-sm font-medium text-gray-700 mb-2">Nội dung:</label>
                        <textarea id="content" name="content" rows="12" required class="form-textarea w-full" placeholder="Nhập nội dung đầy đủ của tin tức bóng đá"><?php echo htmlspecialchars($football_news['content']); ?></textarea>
                    </div>

                    <div>
                        <label for="image_url" class="block text-sm font-medium text-gray-700 mb-2">Hình ảnh (URL):</label>
                        <input type="url" id="image_url" name="image_url" value="<?php echo filter_var($football_news['image'], FILTER_VALIDATE_URL) ? htmlspecialchars($football_news['image']) : ''; ?>" class="form-input w-full" placeholder="Dán URL hình ảnh vào đây">
                    </div>

                    <?php if (!empty($football_news['image'])): ?>
                        <div class="mt-6 flex flex-col sm:flex-row items-center space-y-4 sm:space-y-0 sm:space-x-6 bg-gray-100 p-4 rounded-lg shadow-inner">
                            ...
                            <img src="<?php echo htmlspecialchars($football_news['image']); ?>" alt="Current Image" class="w-40 h-40 object-cover rounded-md shadow-lg border border-gray-300">
                            
                        </div>
                    <?php endif; ?>

                    <div class="flex items-center">
                        <input type="checkbox" id="is_published" name="is_published" value="1" <?php echo $football_news['is_published'] ? 'checked' : ''; ?> class="form-checkbox">
                        <label for="is_published" class="ml-3 block text-base font-medium text-gray-700 cursor-pointer">Đăng tin tức</label>
                    </div>

                    <div class="flex justify-end pt-4">
                        <button type="submit" class="inline-flex items-center px-8 py-4 border border-transparent text-lg font-bold rounded-lg shadow-xl text-white bg-blue-600 hover:bg-blue-700 focus:outline-none focus:ring-4 focus:ring-blue-500 focus:ring-opacity-50 transition duration-300 ease-in-out transform hover:scale-105">
            ...
            <?php else: ?>
                <p class="text-red-600 text-center text-xl font-semibold bg-gray-100 p-6 rounded-lg shadow-md border border-red-300">Không tìm thấy tin tức bóng đá để chỉnh sửa.</p>
            <?php endif; ?>
        </div>
    </div>
</body>
