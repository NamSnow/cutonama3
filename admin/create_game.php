...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!empty($imageUrl)) {
        ...
        $_SESSION['error_message'] = "Vui lòng cung cấp một URL ảnh cho tin tức game.";
        $error = true;
    }
    ...
    if (!$error && (empty($title) || empty($summary) || empty($content) || empty($game_title) || empty($platform))) {
    ...
    if (!$error) {
        try {
            ...
            $_SESSION['success_message'] = "Đã thêm tin tức game thành công!";
            ...
            header("Location: create_game.php"); // Chuyển hướng lại về form với lỗi
            exit();
        }
    } else {
...
<head>
    ...
    <title>Admin - Thêm tin tức Game</title>
...
<body class="bg-gray-100 py-10">
    <div class="max-w-4xl mx-auto p-8 bg-white rounded-xl shadow-2xl">
        <h1 class="text-3xl font-extrabold text-gray-800 mb-8 text-center">Thêm tin tức Game</h1>
        ...
        <form method="POST" action="create_game.php" class="space-y-6">
            <div>
                ...
                <input
                    ...
                    placeholder="Nhập tiêu đề tin tức game"
                >
            </div>
            <div>
                ...
                <textarea
                    ...
                    rows="3"
                    class="shadow-sm appearance-none border border-gray-300 rounded w-full py-3 px-4 text-gray-700 leading-tight focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition duration-200"
                    placeholder="Tóm tắt ngắn gọn nội dung tin tức game"
                ></textarea>
            </div>
            ...
            <div class="bg-gray-50 p-6 rounded-lg border border-gray-200">
                ...
                <input
                    ...
                    required
                    class="shadow-sm appearance-none border border-gray-300 rounded w-full py-3 px-4 text-gray-700 leading-tight focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition duration-200"
                    placeholder="http://example.com/hinh-anh-game.jpg"
                >
                <p class="text-sm text-gray-500 mt-2">Vui lòng dán một URL ảnh hợp lệ (ví dụ: từ các trang tin tức game, thư viện ảnh game).</p>
            </div>

            <div>
                <label for="game_title" class="block text-gray-700 text-sm font-semibold mb-2">Tên Game:</label>
                <input
                    type="text"
                    name="game_title"
                    id="game_title"
                    required
                    class="shadow-sm appearance-none border border-gray-300 rounded w-full py-3 px-4 text-gray-700 leading-tight focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition duration-200"
                    placeholder="Ví dụ: Valorant, Liên Minh Huyền Thoại"
                >
            </div>
            <div>
                <label for="platform" class="block text-gray-700 text-sm font-semibold mb-2">Nền tảng:</label>
                <input
                    type="text"
                    name="platform"
                    id="platform"
                    required
                    class="shadow-sm appearance-none border border-gray-300 rounded w-full py-3 px-4 text-gray-700 leading-tight focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition duration-200"
                    placeholder="Ví dụ: PC, Mobile, PS5, Xbox Series X"
                >
            </div>
