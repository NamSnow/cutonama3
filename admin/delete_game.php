...
if (isset($_GET['id']) && !empty($_GET['id'])) {
    $gameNewsId = intval($_GET['id']); // Sanitize input: ensure it's an integer
    ...
    try {
        ...
        $stmt_select_image = $pdo->prepare("SELECT image FROM game_news WHERE game_news_id = ?");
        $stmt_select_image->execute([$gameNewsId]);
        $gameNews = $stmt_select_image->fetch(PDO::FETCH_ASSOC);

        // 2. Delete the image file from the server if it exists
        if ($gameNews && !empty($gameNews['image'])) {
            // Construct the absolute path to the image file
            // Adjust this path if your image upload directory is different
            $imagePath = __DIR__ . '/../' . $gameNews['image']; 
        ...
        // 3. Delete the game news record from the database
        $stmt_delete = $pdo->prepare("DELETE FROM game_news WHERE game_news_id = ?");
        $stmt_delete->execute([$gameNewsId]);

        // Commit the transaction
        $pdo->commit();

        // Set a success message for the user
        $_SESSION['message'] = 'Tin tức Game đã được xóa thành công.';
        ...
        $_SESSION['message'] = 'Lỗi khi xóa tin tức Game: ' . $e->getMessage();
        $_SESSION['message_type'] = 'error';
        // Log the actual error for debugging
        error_log("Database error during game news deletion: " . $e->getMessage());
    }

} else {
    // If no ID is provided, set an error message
    $_SESSION['message'] = 'Không có ID tin tức Game được cung cấp để xóa.';
    $_SESSION['message_type'] = 'error';
}
