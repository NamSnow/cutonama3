...
require_once '../includes/db.php'; // Đảm bảo đường dẫn tới file kết nối CSDL là đúng

if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $imagePath = null;
    $error = false; // Cờ để kiểm tra lỗi trước khi xử lý

    // Lấy và xác thực URL ảnh
    $imageUrl = trim($_POST['image_url'] ?? '');
    if (!empty($imageUrl)) {
        // Xác thực URL đơn giản. Bạn có thể muốn sử dụng regex mạnh hơn cho môi trường sản phẩm.
        ...
        $_SESSION['error_message'] = "Vui lòng cung cấp một URL ảnh cho tin tức.";
        $error = true;
    }
    ...
    if (!$error && (empty($title) || empty($summary) || empty($content) || empty($match_date) || empty($teams))) {
    ...
    if (!$error) {
        try {
            ...
            $_SESSION['success_message'] = "Đã thêm tin tức bóng đá thành công!";
            header("Location: /cutonama3/admin/index.php"); // Chuyển hướng về trang quản trị chính
            exit();
        } catch (PDOException $e) {
            $_SESSION['error_message'] = "Lỗi khi thêm tin tức: " . $e->getMessage();
            header("Location: create_football.php"); // Chuyển hướng lại về form với lỗi
            exit();
        }
    } else {
...
<head>
    ...
    <title>Admin - Thêm tin tức Bóng đá</title>
    ...
    <style>
        body {
            ...
            font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; /* Font sạch, hiện đại */
        }
        /* Custom styling for CKEditor to blend with Tailwind inputs */
        ...
        /* Basic Tailwind-like styling for jQuery UI Datepicker to override default theme */
        .ui-datepicker {
            @apply shadow-lg rounded-lg bg-white p-4 text-gray-800 border border-gray-200;
        }
        .ui-datepicker .ui-datepicker-header {
            @apply bg-blue-600 text-white p-2 rounded-t-lg flex justify-between items-center;
        }
        .ui-datepicker .ui-datepicker-prev,
        .ui-datepicker .ui-datepicker-next {
            @apply cursor-pointer p-1 rounded-full hover:bg-blue-700 transition-colors;
        }
        ...
        .ui-datepicker th {
            @apply text-gray-500 font-medium text-sm pb-2;
        }
        .ui-datepicker td {
            @apply p-1;
        }
        ...
        .ui-datepicker td a {
            @apply block text-center py-2 px-1 rounded-md text-sm;
        }
        .ui-datepicker td a:hover {
            @apply bg-blue-100 text-blue-800;
        }
        .ui-state-highlight, .ui-widget-content .ui-state-highlight, .ui-widget-header .ui-state-highlight {
            @apply bg-blue-200 text-blue-900 font-semibold;
        }
...
<body class="bg-gray-100 py-10">
    <div class="max-w-4xl mx-auto p-8 bg-white rounded-xl shadow-2xl">
        <h1 class="text-3xl font-extrabold text-gray-800 mb-8 text-center">Thêm tin tức Bóng đá</h1>
        ...
        <form method="POST" action="create_football.php" class="space-y-6">
            <div>
                ...
                <input
                    ...
                    placeholder="Nhập tiêu đề tin tức bóng đá"
                >
            </div>
            <div>
                ...
                <textarea
                    ...
                    rows="3"
                    class="shadow-sm appearance-none border border-gray-300 rounded w-full py-3 px-4 text-gray-700 leading-tight focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition duration-200"
                    placeholder="Tóm tắt ngắn gọn về trận đấu hoặc sự kiện"
                ></textarea>
            </div>
            <div class="ckeditor-container">
                <label for="content" class="block text-gray-700 text-sm font-semibold mb-2">Nội dung chi tiết:</label>
                <textarea
                    name="content"
                    id="content"
                    class="shadow-sm appearance-none border border-gray-300 rounded w-full py-3 px-4 text-gray-700 leading-tight focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition duration-200"
            ...
            <div class="bg-gray-50 p-6 rounded-lg border border-gray-200">
                ...
                <input
                    ...
                    required
                    class="shadow-sm appearance-none border border-gray-300 rounded w-full py-3 px-4 text-gray-700 leading-tight focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition duration-200"
                    placeholder="http://example.com/hinh-anh-bong-da.jpg"
                >
                <p class="text-sm text-gray-500 mt-2">Vui lòng dán một URL ảnh hợp lệ (ví dụ: từ các trang tin tức, thư viện ảnh trực tuyến).</p>
            </div>

            <div>
                <label for="match_date" class="block text-gray-700 text-sm font-semibold mb-2">Ngày diễn ra trận đấu:</label>
                <input
                    type="text"
                    name="match_date"
                    id="match_date"
            ...
            <div>
                <label for="teams" class="block text-gray-700 text-sm font-semibold mb-2">Các đội:</label>
                <input
                    type="text"
                    name="teams"
                    id="teams"
                    required
                    class="shadow-sm appearance-none border border-gray-300 rounded w-full py-3 px-4 text-gray-700 leading-tight focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition duration-200"
                    placeholder="Ví dụ: Đội A vs Đội B"
                >
            </div>
    ...
    <script>
        $( function() {
            ...
            $( "#match_date" ).datepicker({
                ...
                changeMonth: true,
                changeYear: true,
                // Thêm class Tailwind để dễ dàng tùy chỉnh qua CSS bên trên
                // Tailwind CSS classes applied via JS to Datepicker elements
                beforeShow: function(input, inst) {
                    $('#ui-datepicker-div').addClass('shadow-lg rounded-lg border border-gray-200');
                },
                onChangeMonthYear: function(year, month, inst) {
                    // Reapply classes on month/year change if needed
                    $('#ui-datepicker-div').addClass('shadow-lg rounded-lg border border-gray-200');
                }
            });
        } );
